namespace ElgatoLightApiClient.Helpers
{
    internal static class TemperatureConverter
    {
        private static readonly Lazy<Dictionary<UInt16, UInt16>> KelvinToMiredsMap = new(() =>
            new Dictionary<UInt16, UInt16>
            {
                { 7000, 143 },
                { 6950, 144 },
                { 6900, 145 },
        ...
        private static readonly Lazy<Dictionary<UInt16, UInt16>> MiredsToKelvinMap = new(() =>
            KelvinToMiredsMap.Value.ToDictionary(kvp => kvp.Value, kvp => kvp.Key));

        internal static UInt16 MiredsToKelvin(UInt16 mireds)
        {
        ...
        internal static UInt16 KelvinToMireds(UInt16 kelvin)
        {
